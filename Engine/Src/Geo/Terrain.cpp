...
float Terrain::GetHeightAtPt(const float& x, const float& z) const{
	...
	const long long terrainSize = (const long long)sqrt((float)data.size());
	const long long zCoord = (const long long)((z + 0.5) * terrainSize);
	const long long xCoord = (const long long)((x + 0.5) * terrainSize);
...
void Terrain::Create(){
	const long long terrainSize = (const long long)sqrt((double)data.size());
